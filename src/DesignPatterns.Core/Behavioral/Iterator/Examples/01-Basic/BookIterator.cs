...
public class BookCollection
{
    ...
    {
        _books.Add(book);
        Console.WriteLine($"  + {book.Title} agregado");
    }

    public void RemoveBook(Book book)
    {
        _books.Remove(book);
        Console.WriteLine($"  - {book.Title} removido");
    }
    ...
    private class BookIterator : IBookIterator
    {
        ...
        {
            if (!HasNext())
                throw new InvalidOperationException("No hay más libros");
        ...
        {
            _index = 0;
            Console.WriteLine("[BookIterator] Reiniciando");
        }
    }
    ...
    private class ReverseBookIterator : IBookIterator
    {
        ...
        public void Reset()
        {
            _index = _collection.GetBookCount() - 1;
            Console.WriteLine("[ReverseBookIterator] Reiniciando inverso");
        }
    }

    /// <summary>
    /// Iterator filtrado por año
    /// </summary>
    private class BookIteratorByYear : IBookIterator
    {
        ...
        {
            _index = 0;
            Console.WriteLine($"[BookIteratorByYear] Reiniciando (año: {_year})");
        }
    }
}